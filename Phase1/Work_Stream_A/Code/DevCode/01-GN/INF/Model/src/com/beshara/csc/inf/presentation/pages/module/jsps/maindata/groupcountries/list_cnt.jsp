    ...
    <t:panelGroup forceId="true" id="dataT_data_panel" styleClass="dataT-With-1-row-filter">
        <t:dataTable id="dataT_data" var="list"
            ...
            <t:column id="name_column" sortable="false" width="35%">
                <f:facet name="header">
                    <t:commandLink id="sort_countriesDTO-name" forceId="true" styleClass="headerLink"
                                   value="#{resourcesBundle.cntryName}" actionListener="#{pageBeanName.sortDataTable}">
                        <t:graphicImage value="#{(pageBeanName.sortAscending&&pageBeanName.fullColumnName=='countriesDTO-name') ? '/app/media/images/ascending-arrow.gif' : ''}"
                                        border="0"/>
                        <t:graphicImage value="#{(!pageBeanName.sortAscending&&pageBeanName.fullColumnName=='countriesDTO-name') ? '/app/media/images/descending-arrow.gif' :''}"
                                        border="0"/>
                    </t:commandLink>
                </f:facet>
            ...
            <t:column id="captial_column" sortable="false" width="35%">
                <f:facet name="header">
                    <t:commandLink id="sort_captial" forceId="true" styleClass="headerLink"
                                   value="#{resourcesBundle.captial}" actionListener="#{pageBeanName.sortDataTable}">
                        ...
                        <f:param name="elExpression"
                                 value="( !empty groupCountriesListBean.myTableData[groupCountriesListBean.currentSortingRowIndex].countriesDTO.countryCitiesDTOList ? groupCountriesListBean.myTableData[groupCountriesListBean.currentSortingRowIndex].countriesDTO.countryCitiesDTOList[0].name :null)"/>
                        <t:graphicImage value="#{(pageBeanName.sortAscending&&pageBeanName.fullColumnName=='captial') ? '/app/media/images/ascending-arrow.gif' : ''}"
                                        border="0"/>
                        <t:graphicImage value="#{(!pageBeanName.sortAscending&&pageBeanName.fullColumnName=='captial') ? '/app/media/images/descending-arrow.gif' :''}"
                                        border="0"/>
                    </t:commandLink>
                </f:facet>
                <h:outputText id="content_captial" value="#{list.countriesDTO.countryCitiesDTOList[0].name}"/>
            </t:column>
            <t:column id="status_column" sortable="false" width="19%" styleClass="standardTable_ColumnCentered">
                <f:facet name="header">
                    <%-- <t:commandLink id="sort_reqSerial" forceId="true" styleClass="headerLink"
                         value="#{resourcesBundle.nationality}" actionListener="#{pageBeanName.sortDataTable}">
                         <t:graphicImage value="#{(pageBeanName.sortAscending&&pageBeanName.fullColumnName=='reqSerial')
                         ? '/app/media/images/ascending-arrow.gif' :''}" border="0"/> <t:graphicImage
                         value="#{(!pageBeanName.sortAscending&&pageBeanName.fullColumnName=='reqSerial') ?
                         '/app/media/images/descending-arrow.gif' :''}" border="0"/> </t:commandLink>--%>
                    <t:outputLabel value="#{resourcesBundle.nationality}"/>
                </f:facet>
                <t:graphicImage value="/app/media/images/icon_details_number.gif"
                                onmouseover="doTooltip(event,' #{resourcesBundle.TooltipMale}(#{list.countriesDTO.genderCountryDTOList[1].name}) &lt;br> #{resourcesBundle.TooltipFamale}(#{list.countriesDTO.genderCountryDTOList[0].name})')"
                                onmouseout="hideTip()" title="#{resourcesBundle.nationality}"/>
            </t:column>
        </t:dataTable>
