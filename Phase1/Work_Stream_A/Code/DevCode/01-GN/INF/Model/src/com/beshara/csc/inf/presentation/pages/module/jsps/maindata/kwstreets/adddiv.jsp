...
<t:panelGroup forceId="true" id="divAddLookup">
    ...
    <h:outputText id="clientErrorMessage" styleClass="errMsg" />
        ...
        <t:panelGrid columns="2" border="0" styleClass="row01" width="100%" rowClasses="row01,row02" cellpadding="0" cellspacing="0">
            ...
            <t:panelGroup>
                <t:inputText maxlength="#{pageBeanName.nameMaxLength}" id="kwStreetsName" value="#{pageBeanName.pageDTO.name}" styleClass="textboxlarge" forceId="true" />
                <h:outputText value="*" styleClass="mandatoryAsterisk"/>
                <c:requiredFieldValidator componentToValidate="kwStreetsName" display="dynamic" errorMessage="#{globalResources.missingField}" highlight="false" active="#{kwStreetsBean.pageMode==1}"/>
            </t:panelGroup>
          
            <%--<t:outputText value="#{resourcesBundle.holtypeDays}"  styleClass="lable01"/>--%>
            <%--<t:panelGroup>
                <t:inputText maxlength="#{pageBeanName.nameMaxLength}"  id="holtypeDays" value="#{pageBeanName.pageDTO.holtypeDays}" styleClass="textboxlarge" forceId="true" />
                <h:outputText value="*" styleClass="mandatoryAsterisk"/>
                <c:requiredFieldValidator componentToValidate="holtypeDays"  display="dynamic" errorMessage="#{globalResources.missingField}" highlight="false" active="#{holidayTypesBean.pageMode==1}"/>
...
<t:panelGrid columns="3" border="0" align="center" style="margin-top:10px;">
            ...
            <h:panelGroup>
                ...
                <a4j:jsFunction name="saveAndNew" action="#{pageBeanName.saveAndNew}" reRender="divAddLookup" oncomplete="setFocusOnlyOnElement('kwStreetsName');"/>
            </h:panelGroup>
            <h:panelGroup>
                ...
                <a4j:jsFunction name="backJsFunction" action="#{pageBeanName.cancelAdd}" oncomplete="hideLookUpDiv(window.blocker,window.lookupAddDiv,'myForm:kwStreetsName','myForm:error','add');foucsAddbuttonOnList();" reRender="editButton,divAddLookup,dataT_data_panel,noOfRecords,paging_panel,listSize"/>
            </h:panelGroup>
</t:panelGrid>
