...
<t:panelGroup forceId="true" id="divEditLookup">
    ...
    <h:panelGrid columns="1" rendered="#{pageBeanName.showEdit}" width="100%">
        <h:panelGrid columns="2" border="0" rowClasses="row02,row01" width="100%" cellpadding="3" cellspacing="0">
            ...
            <h:panelGroup>
                <t:inputText id="holtypeNameEdit" value="#{pageBeanName.selectedPageDTO.name}" styleClass="textboxlarge"
                             maxlength="#{pageBeanName.nameMaxLength}" forceId="true"/>
                <h:outputText value="*" styleClass="mandatoryAsterisk"/>
                <c:requiredFieldValidator componentToValidate="holtypeNameEdit" display="dynamic"
                                          errorMessage="#{globalResources.missingField}" highlight="false"
                                          active="#{holidayTypesBean.pageMode==2}"/>
            </h:panelGroup>
            <t:outputText value="#{resourcesBundle.holtypeDays}" styleClass="lable01"/>
            <t:panelGroup>
                <t:inputText id="holtypeDaysEdit" value="#{pageBeanName.selectedPageDTO.holtypeDays}"
                             onkeypress="enabelFloatOnly(this);" onkeyup="enabelFloatOnly(this);"
                             styleClass="textboxlarge" maxlength="2" forceId="true"/>
                <h:outputText value="*" styleClass="mandatoryAsterisk"/>
                <c:requiredFieldValidator componentToValidate="holtypeDaysEdit" display="dynamic"
                                          errorMessage="#{globalResources.missingField}" highlight="false"
                                          active="#{holidayTypesBean.pageMode==2}"/>
                <c:regularExpressionValidator componentToValidate="holtypeDaysEdit" pattern="/^[0-9]/" display="dynamic"
                                              ...
                                              highlight="false" active="#{holidayTypesBean.pageMode==2}"/>
            </t:panelGroup>
        </h:panelGrid>
        <t:panelGrid columns="3" border="0" align="center" style="margin-top:10px;">
            ...
            <t:panelGroup>
                <t:commandButton forceId="true" id="CancelButtonEdit" styleClass="cssButtonSmall"
                                 value="#{globalResources.back}" onblur="setFocusOnlyOnElement('holtypeNameEdit');"/>
                <a4j:jsFunction name="cancelEditJs" action="#{pageBeanName.cancelEdit}"
                                oncomplete="hideLookUpDiv(window.blocker,window.lookupEditDiv,'holtypeNameEdit','myForm:errorEdit');"
