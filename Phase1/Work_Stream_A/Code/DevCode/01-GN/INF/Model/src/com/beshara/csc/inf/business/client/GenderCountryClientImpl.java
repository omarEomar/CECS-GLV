...
import com.beshara.csc.inf.business.deploy.GenderCountrySession;
 ...
 * @since 03/09/2007
 */
public class GenderCountryClientImpl extends BaseClientImpl3 implements IGenderCountryClient {
 
    public GenderCountryClientImpl() {
    }

    @Override
    protected Class<? extends BasicSession> getSessionInterface() {
        return GenderCountrySession.class;
    }

    @Override
    protected GenderCountrySession SESSION() {
        return (GenderCountrySession)super.SESSION();
    }


    /**
     * List gender names by country code * @param countryCode
    ...
    public List<IBasicDTO> getGenderNames(Long countryCode) throws DataBaseException, SharedApplicationException {
        try {
            return SESSION().getGenderNames(RI(), countryCode);
    ...
    public List<IBasicDTO> getCountriesByGenderType(Long genderType) throws DataBaseException, SharedApplicationException {
        try {
            return SESSION().getCountriesByGenderType(RI(), genderType);
        } catch (SharedApplicationException e) {
            throw e;
        } catch (Exception e) {
            throw getWrappedException(e);
