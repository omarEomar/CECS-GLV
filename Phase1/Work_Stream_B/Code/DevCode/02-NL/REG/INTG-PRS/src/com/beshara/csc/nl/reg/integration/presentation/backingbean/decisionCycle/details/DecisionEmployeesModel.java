...
import com.beshara.csc.nl.reg.integration.presentation.backingbean.decisionCycle.AbstractDecisionListBean;
...
public class DecisionEmployeesModel extends ManyToManyDetailsMaintain {
    ...
    public DecisionEmployeesModel() {
        setClient(RegClientFactory.getEmpDecisionsClient());
    }

    public void navigateToNextPage(Long pageNo) {
        DecisionCycleEmployeesMaintain decisionEmployeesMaintainBean = 
            (DecisionCycleEmployeesMaintain)evaluateValueBinding("decisionCycleEmployeesMaintainBean");
        DecisionCycleMaintainBean decisionBean = (DecisionCycleMaintainBean)BeansUtil.getValue("decisionCycleMaintainBean");
        if (getOriginalMap().containsKey(pageNo)) {
            List objList = getOriginalMap().get(pageNo);
            for (int i = 0; i < objList.size(); i++) {
                getAddElementMap().put(((IEmpDecisionsEntityKey)((IEmpDecisionsDTO)objList.get(i)).getCode()).getCivilId(), 
                                       (IEmpDecisionsDTO)objList.get(i));
            ...
            try {
                ...
                List list = RegClientFactory.getDecisionsClient().getEmpDecisionsDTOListByPage(decisionBean.getPageDTO().getCode(), 
                                                                                               indexOfDB);
                if (list != null) {
                    getOriginalMap().put(pageNo, list);
                    for (int i = 0; i < list.size(); i++) {
                        getAddElementMap().put(((IEmpDecisionsEntityKey)((IEmpDecisionsDTO)list.get(i)).getCode()).getCivilId(), 
                                               (IEmpDecisionsDTO)list.get(i));
    ...
    public int determineNoOfPaqe() {
        DecisionCycleEmployeesMaintain decisionEmployeesMaintainBean = 
            (DecisionCycleEmployeesMaintain)evaluateValueBinding("decisionCycleEmployeesMaintainBean");
        int newPage = 
            (decisionEmployeesMaintainBean.getCurrentListSize() / 11) + 1;
        return newPage;
    }
    ...
    public boolean checkExistData(IBasicDTO dto) {
        ...
        if (getAddElementMap().containsKey(civil)) {
            ...
            try {
                exist = RegClientFactory.getEmpDecisionsClient().checkEixstEmpInDecision(decisionBean.getPageDTO().getCode(), 
                                                                                         ((IEmployeesEntityKey)((IEmployeesDTO)dto).getCode()).getCivilId());
            } catch (SharedApplicationException e) {
                exist = false;
                e.printStackTrace();
            } catch (DataBaseException e) {
    ...
    public void addNewElement(IBasicDTO dto) {
        ...
        if (getOriginalMap().containsKey(new Long(newPage))) {
            ...
            getAddElementMap().put(new Long(((EmpDecisionsEntityKey)dto.getCode()).getCivilId()), 
                                   dto);
        } else {
            objList.add(dto);
            getAddElementMap().put(new Long(((EmpDecisionsEntityKey)dto.getCode()).getCivilId()), 
        ...
        if (decisionMaintainBean.getMaintainMode() == 1L) {
            setNewCurrentListSize(decisionEmployeesMaintainBean.getCurrentListSize() + 
                                  1);
        } else {
    ...
    public void sendListtoBussiens() throws Exception {
        ...
        DecisionCycleEmployeesMaintain decisionEmployeesMaintainBean = 
            (DecisionCycleEmployeesMaintain)evaluateValueBinding("decisionCycleEmployeesMaintainBean");
        AbstractDecisionListBean decisionListBean = (AbstractDecisionListBean)BeansUtil.getValue("decisionAbstractListBean");
        ...
        for (int i = 0; i < determineNoOfPaqe(); i++) {
            ...
            if (list != null) {
                for (int y = 0; y < list.size(); y++) {
                    if (((IEmpDecisionsDTO)list.get(y)).getStatusFlag() == 
                        null) {
                    ...
                    if (((IEmpDecisionsDTO)list.get(y)).getStatusFlag() != 
                        ...
                        if (((IEmpDecisionsDTO)list.get(y)).getStatusFlag() != 
                            4L && 
                            ((IEmpDecisionsDTO)list.get(y)).getStatusFlag() != 
                            5L) {
                            empList.add((IEmpDecisionsDTO)list.get(y));
                        }
                    }
        ...
        for (int z = 0; z < empList.size(); z++) {
            if (((IEmpDecisionsDTO)empList.get(z)).getStatusFlag() != null) {
                if (((IEmpDecisionsDTO)empList.get(z)).getStatusFlag() == 1L || 
                    ((IEmpDecisionsDTO)empList.get(z)).getStatusFlag() == 5L) {
                    deleteItem = deleteItem + 1;
                }
            }
        }
        if (deleteItem == 
            decisionEmployeesMaintainBean.getCountOfAllEmpDecision()) {
            if (decisionListBean.isCancelDescisionFlag() == false) {
                getSharedUtil().handleException(new Exception(), 
                                                ...
                                                "no_employess_Added");
                decisionEmployeesMaintainBean.setShowErrMsg(true); 
                throw new Exception();
            }
        }
