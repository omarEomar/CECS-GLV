...
public class DecisionCycleMainDataMaintain extends ManyToManyDetailsMaintain {
    ...
    private static final String BEAN_NAME = "financeDecisionCycleMainDataMaintainBean";
...
//    private String selectedFormRadio;
 ...
 //   private int indexOfSelectedDataInFormsDataTable;
    ...
    public static DecisionCycleMainDataMaintain getInstance() {
        ...
        return (DecisionCycleMainDataMaintain)app.createValueBinding("#{" + BEAN_NAME + "}").getValue(ctx);
    }
...
//        app.setShowLookupEdit(true);
    ...
    public void initiateBeanOnce(){
     //   secWorkCenterUsersSearchDTO = SecDTOFactory.createSecWorkCenterUsersSearchDTO();
        ...
        itemSelectionRequiredValue = ISystemConstant.VIRTUAL_VALUE;
        itemSelectionRequiredValueString = itemSelectionRequiredValue.toString();
        ...
        if (statusDTOKey == null || statusDTOKey.length() == 0 ||
            ...
            statusDTOKey = (String)BeansUtil.getValue("financeDecisionCycleMaintainBean.pageDTO.regStatusDTO.code.key");
        }
        //**********************
        // types
        ...
        //////////////////////////
        if ((Boolean)BeansUtil.getValue("financeDecisionCycleMaintainBean.maintainMode==1")
            || (Boolean)BeansUtil.getValue("financeDecisionCycleMaintainBean.maintainMode==2") ) {
            /*edit or view*/
            // Load on startup only in Edit Mode
        ...
        if (subjects == null || subjects.size() == 0) {
            ...
            try{
                subjects = RegClientFactory.getSubjectsClient().getCodeNameByAppModuleAndSysSettingCode(IConstants.SYSTEM_SETTING_FINANCIAL_MERITS_CODE, IConstants.APP_MOUDLE_REG_CODE);
                /* in view mode add subject of */
                if( (Boolean)BeansUtil.getValue("financeDecisionCycleMaintainBean.showOnly") ||
                    ...
                    if(subjectCode2 != null){
                        List subjects2 = RegClientFactory.getSubjectsClient().getCodeNameBySubjectCode(subjectCode2);
                        if(subjects2 != null)subjects.addAll(subjects2);                                                                                                                                                                                  
                    }
                }
            }catch(Exception e){
                e.printStackTrace();
        ...
        //***********************************
        //firstSubject
        if((Boolean)BeansUtil.getValue("financeDecisionCycleMaintainBean.maintainMode==0")){
...
//        getLovBaseBean().setReturnMethodName("financeDecisionCycleMainDataMaintainBean.addDecisionType");
//        getLovBaseBean().setSearchMethod("financeDecisionCycleMainDataMaintainBean.searchDecisionType");
...
//        getLovBaseBean().setCancelSearchMethod("financeDecisionCycleMainDataMaintainBean.cancelSearchLovDiv");
//    }
...
//        getLovBaseBean().setReturnMethodName("financeDecisionCycleMainDataMaintainBean.addDecisionSubject");
//        getLovBaseBean().setSearchMethod("financeDecisionCycleMainDataMaintainBean.searchDecisionSubject");
//        getLovBaseBean().setSelectedDTOS(new ArrayList());
//        getLovBaseBean().setSelectedRadio("");
//        getLovBaseBean().setSearchTyp("0");
//        getLovBaseBean().setSearchQuery("");
...
//    }
    ...
    public void resetDecMkrSignBy(ActionEvent event) throws DataBaseException {
        ...
        try {
            DecisionMakerTypesDTO decisionMakerTypesDTO = (DecisionMakerTypesDTO)InfClientFactory.getDecisionMakerTypesClient().getById(
                InfEntityKeyFactory.createDecisionMakerTypesEntityKey(Long.parseLong(decisionMakerDTOCode) ) );
    ...
    public void filterDecMkrBytype(ActionEvent event) throws DataBaseException {
        ...
        String typesKey = ((IDecisionsDTO)decisionCycleMaintainBean.getPageDTO()).getTypesDTOKey();
        Long minCode = CSCSecurityInfoHelper.getLoggedInMinistryCode();
        ((IDecisionsDTO)decisionCycleMaintainBean.getPageDTO()).setDecisionMakerDTOKey(null);
        if (typesKey == "" || typesKey == null) {
            setDecMkrDisabled(true);
            decisionMakers = new ArrayList<IBasicDTO>();
...
//        if (subjects == null || subjects.size() == 0) {
//            //subjects = RegClientFactory.getSubjectsClient().getCodeNameByModuleCode(null, IConstants.APP_MODULE_CODE_FOR_MER);
//            
//            if (subjects == null)
//                subjects = new ArrayList<IBasicDTO>();
//            subjects.add(((IDecisionsDTO)getMaintainBean().getPageDTO()).getSubjectsDTO());
//        }
//        return subjects;
//    }
    
    private DecisionCycleMaintainBean getMaintainBean(){
        return (DecisionCycleMaintainBean)evaluateValueBinding("financeDecisionCycleMaintainBean");
    }
            ...
            if (!(typesDTOKey == null || typesDTOKey.length() == 0 ||
                ...
                for (IBasicDTO type : getTypes()) {
                    if (type.getCode().getKey().equals(typesDTOKey)) {
                        ...
                        BeansUtil.setValue("financeDecisionCycleMaintainBean.pageDTO.typesDTO", typesDto);
        ...
        if (!(yearsDTOKey == null || yearsDTOKey.length() == 0 ||
            ...
            for (IYearsDTO year : getIssuanceYears()) {
                if (year.getCode().getKey().equals(yearsDTOKey)) {
                    yearsDto = year;
                    BeansUtil.setValue("financeDecisionCycleMaintainBean.pageDTO.yearsDTO", yearsDto);
...
//    }
  ...
  /*  
    public void openSearchForms(ActionEvent ae) {
        ...
        cancelSearchForms();
        try {
            formsList = ((IDecisionsClient)getClient()).getDecisionsForms(new Long(1), CSCSecurityInfoHelper.getLoggedInMinistryCode());
        } catch (DataBaseException e) {
            e.printStackTrace();
        }
...
//    }
    ...
    * @author Amr Abdel Halim
    * @since 07/10/2015
    * @Note Updated to enhance performance
    */
        ...
        if (!(statusDTOKey == null || statusDTOKey.length() == 0 ||
            ...
            for (IRegulationStatusDTO status : getRegStatusDTOList()) {
                if (status.getCode().getKey().equals(statusDTOKey)) {
                    BeansUtil.setValue("financeDecisionCycleMaintainBean.pageDTO.regStatusDTO", status);
                    break;
                }
            }
        }
