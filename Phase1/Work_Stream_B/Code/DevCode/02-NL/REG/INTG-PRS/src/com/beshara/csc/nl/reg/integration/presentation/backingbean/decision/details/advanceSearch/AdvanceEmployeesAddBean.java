package com.beshara.csc.nl.reg.integration.presentation.backingbean.decision.details.advanceSearch;
...
public class AdvanceEmployeesAddBean extends ManyToManyDetailsMaintain {
    ...
    private List<ICategoryInfo> categories = new ArrayList<ICategoryInfo>();
    private List<IMinistryInfo> ministries = new ArrayList<IMinistryInfo>();
    ...
    private int divMode = 0;
    private static final int DIV_MODE_WORK_CENTER = 1;
    private static final int DIV_MODE_JOB = 2;
    ...
    private static final String completeSearchPath = "/module/jsps/shared/employeesearch/employeeadd.jsp";
    private static final String resultSearchPath = "/module/jsps/shared/employeesearch/searchresult.jsp";
    ...
    public AdvanceEmployeesAddBean() {
    ...
    public AppMainLayout appMainLayoutBuilder() {
        ...
        layout.setShowsteps(false);
        if ((showWorkCenterLovDiv || showJobLovDiv) &&
            (FacesContext.getCurrentInstance().getViewRoot().getViewId().equals(completeSearchPath)))
            layout.setShowLovDiv(true);
        if (!showResultWithinPage &&
            (FacesContext.getCurrentInstance().getViewRoot().getViewId().equals(completeSearchPath))) {
            layout.setShowdatatableContent(false);
            layout.setShowNavigation(true);
        } else if (!showResultWithinPage &&
                   (FacesContext.getCurrentInstance().getViewRoot().getViewId().equals(resultSearchPath))) {
    ...
    public void openWorkCenter() {
        setDivMode(DIV_MODE_WORK_CENTER);
        ...
        getLovBaseBean().setReturnMethodName("advanceEmployeesAddBean.addWorkCenter");
        getLovBaseBean().setSearchMethod("advanceEmployeesAddBean.searchWorkCenter");
        ...
        getLovBaseBean().setCancelSearchMethod("advanceEmployeesAddBean.cancelWorkSearch");
    }
     ...
     */
    public void openJob() {
        setDivMode(DIV_MODE_JOB);
        getLovBaseBean().setMyTableData(getTechnicalJobs());

        getLovBaseBean().getOkCommandButton().setReRender("employees_special_job");
        getLovBaseBean().setReturnMethodName("advanceEmployeesAddBean.addJob");
        getLovBaseBean().setSearchMethod("advanceEmployeesAddBean.searchJob");
        getLovBaseBean().setSelectedDTOS(new ArrayList());
        getLovBaseBean().setSelectedRadio("");
        getLovBaseBean().setSearchTyp("0");
        getLovBaseBean().setSearchQuery("");
        getLovBaseBean().setSearchMode(false);
        getLovBaseBean().setCancelSearchMethod("advanceEmployeesAddBean.cancelJobSearch");
    }
        ...
        if (searchQuery != null && !searchQuery.toString().equals("") && searchType != null &&
            ...
            try {
                ...
                if (searchType.toString().equals("0")) {
                    jobSearchDTO.setJobKey(searchQuery.toString());
    ...
    public void fillMinistries() {
        ...
        try {
            if (categoryID != null)
                for (ICategoryInfo iCategoryInfo : categories) {
                    if (iCategoryInfo.getCode().equals(categoryID)) {
            ...
            ministries = new ArrayList<IMinistryInfo>();
            e.printStackTrace();
        }
        if (ministries == null)
            ministries = new ArrayList();
    ...
    public List<IBasicDTO> getWorkMinistries() {
        if (!showCategoryCB && !showMinistryCB && ((workMinistries == null) || (workMinistries.size() == 0))) {
            ministryID = getManagedConstantsBean().getMinCode();
            fillWorkMinistries();
        }
    ...
    public void fillBranches() {
        try {
            if (bankID != null && !bankID.equals("")) {
                ...
                employeeSearchDTO.setBankCode(Long.valueOf(bankID));
            } else {
                branches = new ArrayList<IBasicDTO>();
                employeeSearchDTO.setBankCode(null);
            }
        } catch (DataBaseException e) {
            branches = new ArrayList<IBasicDTO>();
            employeeSearchDTO.setBankCode(null);
    ...
    public void filterByCader() {
        ...
        if (selectedCaderCode != null && !selectedCaderCode.equals("")) {
            try {
                ...
                jobGroupes =
                        SalClientFactory.getSalElementGuidesClient().getGroupCodeName(Long.valueOf(selectedCaderCode));
                selectedGroupCode = "";
                employeeSearchDTO.setElmguideCode(null);
                employeeSearchDTO.setCaderCode(Long.valueOf(selectedCaderCode));
                currentDegrees = new ArrayList();
            } catch (DataBaseException e) {

                selectedGroupCode = "";
                employeeSearchDTO.setElmguideCode(null);
                employeeSearchDTO.setCaderCode(null);
    ...
    public void obtainDegreeByGroup() {
        ...
        if (selectedGroupCode != null && !selectedGroupCode.equals("")) {
            try {
                currentDegrees =
                        SalClientFactory.getSalElementGuidesClient().getDegreesCodeNameByGroup(Long.valueOf(selectedGroupCode));
                employeeSearchDTO.setGroupCode(Long.valueOf(selectedGroupCode));
            } catch (DataBaseException e) {
                currentDegrees = new ArrayList();
                employeeSearchDTO.setGroupCode(null);
                e.printStackTrace();
            } catch (Exception e) {
                currentDegrees = new ArrayList();
                employeeSearchDTO.setGroupCode(null);
    ...
    public void searchAvailable() throws DataBaseException, SharedApplicationException {
        ...
        if (!showResultWithinPage) {
            getSharedUtil().handleNavigation("Advanced_Result_Emp_Search_Page");
        }
    }
    ...
    private com.beshara.base.paging.impl.PagingResponseDTO getSearchByEmpWithPaging(PagingRequestDTO pagingRequestDTO) {
        ...
        try {
            ...
            //                    (com.beshara.base.paging.impl.PagingResponseDTO)();
            List<IBasicDTO> list =
                EmpClientFactory.getEmployeesClient().filterAvailableEntitiesUsingPaging(getPagingRequestDTO().getSearchDTO(),
                                                                                         bsnPagingRequestDTO);
            Long totalSize =
                EmpClientFactory.getEmployeesClient().filterAvailableEntitiesUsingPagingCount(getPagingRequestDTO().getSearchDTO());
            bsnPagingResponseDTO = new com.beshara.base.paging.impl.PagingResponseDTO();
            bsnPagingResponseDTO.setBasicDTOList(list);
            if (totalSize != null) {
                bsnPagingResponseDTO.setCount(totalSize);
            } else {
                bsnPagingResponseDTO.setCount(Long.valueOf(0));
            }
    ...
    private com.beshara.base.paging.impl.PagingResponseDTO getSearchByInfWithPaging(PagingRequestDTO pagingRequestDTO) {
        ...
        try {
            bsnPagingResponseDTO =
                    (com.beshara.base.paging.impl.PagingResponseDTO)(((IKwtCitizensResidentsClient)InfClientFactory.getKwtCitizensResidentsClientForCenter()).filterAvailableInfUsingPaging(getPagingRequestDTO().getSearchDTO(),
                                                                                                                                                                                            bsnPagingRequestDTO));
    ...
    public void openConfirmMsgDiv() {
        if (CheckEnterHRInformation() == false && CheckEnterSalInformation() == false) {
            setSearchInf(true);
    ...
    public boolean CheckEnterHRInformation() {
        if (employeeSearchDTO.getMinistryFileNo() != null || employeeSearchDTO.getCscFileNo() != null ||
            employeeSearchDTO.getHireDate() != null || employeeSearchDTO.getEmpHireTypes() != null ||
            employeeSearchDTO.getStartWorkingDate() != null || employeeSearchDTO.getTechJobCode() != null) {
            return true;
        } else {
            return false;
        }
    ...
    public boolean CheckEnterSalInformation() {
        if (employeeSearchDTO.getBankbranchCode() != null || employeeSearchDTO.getElmguideCode() != null ||
    ...
    public String backFromResultSearch() {
        setBackBtnClicked(true);
    ...
    public String cancelSearchMethod() {
        ...
        kuwityType = KUWITY;
        employeeSearchDTO.setKuwaiti(true);
        if ((getCancelSimpleSearchMethod() == null) || getCancelSimpleSearchMethod().equals(""))
            return "Advanced_Search_Employees";
        else
            return (String)executeMethodBinding(getCancelSimpleSearchMethod(), null);
    }
    ...
    //            exist = decisionEmployeesModelSessionBean.checkExistData(availableDTO);
    //            if (exist) {
    //                if (availableDTO.getStatusFlag() != null &&
    //                    availableDTO.getStatusFlag().longValue() != getDeleted().longValue()) {
    ...
    public boolean isDivModeWorkCenter(){
      return divMode == DIV_MODE_WORK_CENTER;   
    }
    
    public boolean isDivModeJob(){
        return divMode == DIV_MODE_JOB;  
    }
}
