...
public class TreeDivBean extends TreeBaseBeanMany {
    ...
    public String searchinTree() {
        ...
        try {
            ...
            if (getSearchQuery() != null && !getSearchQuery().equals("")) {
                ...
                SubjectMaintain   subjectMaintain= (SubjectMaintain)evaluateValueBinding("subjectMaintain");
                    for (int j = 0; j < subjectMaintain.getCurrentDetails().size(); j++) {
                        IBasicDTO dto = subjectMaintain.getCurrentDetails().get(j);
                        IBasicDTO codeNameDTO = subjectMaintain.mapToCodeNameDTO(dto);
                        for (int i = 0; i < result.size(); i++) {
                            IBasicDTO resultListDTO = (IBasicDTO)result.get(i);
                            if (codeNameDTO.getCode().getKey().equals(resultListDTO.getCode().getKey())) {
                            if(((ISubjectsDTO)resultListDTO).getLeafFlag()==1)
                                result.remove(resultListDTO);
                            }
                        }
                    }
    ...
    public PagingResponseDTO getChildrenNodes(PagingRequestDTO pagingRequest) throws DataBaseException, 
        ...
        SubjectMaintain subjectMaintain = 
            (SubjectMaintain)evaluateValueBinding("subjectMaintain");
        if (subjectMaintain.getCurrentDetails() != null && resultList != null)
            for (int j = 0; j < subjectMaintain.getCurrentDetails().size();j++) {
                IBasicDTO dto = subjectMaintain.getCurrentDetails().get(j);
                IBasicDTO codeNameDTO = subjectMaintain.mapToCodeNameDTO(dto);
                ...
                for (int i = 0; i < resultList.size(); i++) {
                    IBasicDTO resultListDTO = (IBasicDTO)resultList.get(i);
                    if (((ISubjectsDTO)codeNameDTO).getParentCode() != null) {
                        if (((ISubjectsDTO)codeNameDTO).getParentCode().getKey().equals(resultListDTO.getCode().getKey())) {
                            int z = 
                                ((ISubjectsDTO)resultList.get(i)).getChildernNumbers().intValue();
                            if (z != 0) {
                                ((ISubjectsDTO)resultList.get(i)).setChildernNumbers(new Long(z - 
                                                                                              1));
                            }
                        }
                    }
                    if (codeNameDTO.getCode().getKey().equals(resultListDTO.getCode().getKey())) {
                        ...
                        if (dto.getStatusFlag() != null && 
                            ...
                            if (((IREGAdminJusticeSubjectsDTO)dto).getSubjectsDTO().getLeafFlag() == 
                                1) {
                                resultList.remove(resultListDTO);
                                if (resultList.size() == 0) {
                                    for (int k = 0; 
                                         k < subjectMaintain.getAvailableDetails().size(); 
                                         k++) {
                                        if (subjectMaintain.getAvailableDetails().get(k).getCode().getKey().equals(((ISubjectsDTO)resultListDTO).getParentCode().getKey())) {
                                            ((ITreeDTO)subjectMaintain.getAvailableDetails().get(k)).setChildernNumbers(0l);
                                            setMyTableData(subjectMaintain.getAvailableDetails());
                                        }
                                    }
                                }
                            }
                ...
                if (!exist && dto.getStatusFlag() != null && 
                    ...
                    if (isSearchMode() == true && 
                        subjectMaintain.containSubString(codeNameDTO.getName())) {
                        resultList.add(codeNameDTO);
