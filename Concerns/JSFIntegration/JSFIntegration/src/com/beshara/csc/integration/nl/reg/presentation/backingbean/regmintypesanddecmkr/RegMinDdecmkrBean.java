...
import com.beshara.csc.nl.reg.business.client.IRegMinDecmkrTypesClient;
import com.beshara.csc.nl.reg.business.client.IRegMinRegTypesClient;
import com.beshara.csc.nl.reg.business.client.RegClientFactory;
import com.beshara.csc.nl.reg.business.dto.IRegMinDecmkrTypesDTO;
...
import com.beshara.csc.nl.reg.business.dto.RegMinDecmkrTypesDTO;
...
public class RegMinDdecmkrBean extends LookUpBaseBean {
        private String ministryName = null;
        private String catName = null;
        private IMinistriesDTO ministryDTO; //= OrgDTOFactory.createMinistriesDTO();
        private static final String BACK_BEAN_NAME = "regMinDdecmkrBean";
    public RegMinDdecmkrBean() {
  

        setPageDTO(new RegMinDecmkrTypesDTO()); //set this the page dto
        setSaveSortingState(true);
        super.setSelectedPageDTO(new RegMinDecmkrTypesDTO());
        //IRegModuleSubjectsDTO
        setClient((IRegMinDecmkrTypesClient)RegClientFactory.getRegMinDecmkrTypesClient());
        ...
        public List getBaseSearchResult() throws DataBaseException {
            ...
            try {
                if (getSearchType().intValue() == 0) {
                    searchResult = 
                            RegClientFactory.getRegMinDecmkrTypesClient().SearchByCodeAndMinCode((Long)getSearchEntityObj(), 
                                                                                     Long.parseLong(getMinistryDTO().getCode().getKey()));
                } else if (getSearchType().intValue() == 1) {
                    searchResult = 
                            RegClientFactory.getRegMinDecmkrTypesClient().SearchByNameAndMinCode(formatSearchString(getSearchQuery()), 
                                                                                     Long.parseLong(getMinistryDTO().getCode().getKey()));
                }
    ...
    public void openListOfValuesDiv() {
        ...
        try {
               ...
               RegClientFactory.getRegMinDecmkrTypesClient().getAll();
            } 
    ...
    private void getLoveList() {
        try {
            getLovBaseBean().setMyTableData(RegClientFactory.getRegMinDecmkrTypesClient().listAvailableDecmkrTypes(((IMinistriesEntityKey)getMinistryDTO().getCode()).getMinCode()));
        }
    ...
    public String returnLevelsLovDiv() {
        ...
        try {
            ...
            for (IBasicDTO dto: getLovBaseBean().getSelectedDTOS()) {
                IRegMinDecmkrTypesDTO _regMinDecmkrTypesDTO = 
                    RegDTOFactory.createRegMinDecmkrTypesDTO();
                _regMinDecmkrTypesDTO.setDecmkrtypeDTO(dto);
                _regMinDecmkrTypesDTO.setMinCode(getManagedConstantsBean().getMinCode());
                insertableDTOS.add(_regMinDecmkrTypesDTO);
                
            }
            String message = getSharedUtil().messageLocator("com.beshara.csc.nl.reg.presentation.resources.reg","massgeConetionSuccess");
            RegClientFactory.getRegMinDecmkrTypesClient().add(insertableDTOS);
            getAll();
            this.setSearchQuery("");
    ...
    public void searcher(Object searchType, Object searchQuery) {
        getLovBaseBean().unCheck();  
        getSelectedDTOS().clear();
        Long MinCode = (((IMinistriesEntityKey)getMinistryDTO().getCode()).getMinCode());
        try {
            if (searchQuery != null && !searchQuery.equals("")) {
                if (searchType.toString().equals("0")) {

                    getLovBaseBean().setMyTableData(RegClientFactory.getRegMinDecmkrTypesClient().searchAvailableByCode(new Long(searchQuery.toString()),MinCode));
                    getLovBaseBean().unCheck();                                                                                            
                } else if (searchType.toString().equals("1")) {
                    getLovBaseBean().setMyTableData(RegClientFactory.getRegMinDecmkrTypesClient().searchAvailableByName(searchQuery.toString(),MinCode));
                    getLovBaseBean().unCheck();  
                }
            } 
    ...
    public List getTotalList() {
        ...
        try {
            ...
            totalList = 
                    RegClientFactory.getRegMinDecmkrTypesClient().getAllByMinCode(((IMinistriesEntityKey)getMinistryDTO().getCode()).getMinCode());
