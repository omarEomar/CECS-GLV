...
<t:panelGroup forceId="true" id="divEditLookup">
    ...
    <t:panelGroup rendered="#{detailBeanName.editDivShown}">
        ...
        <t:panelGrid columns="2" border="0" rowClasses="row01,row02"  width="100%" cellpadding="3" cellspacing="0" >
            ...
            <t:panelGroup>
                <t:inputText onblur="document.getElementById('edit_inputTxtArea').focus();" onchange="trimBorders(edit_first_inputTxt);"  forceId="true" maxlength="#{pageBeanName.nameMaxLength}" id="edit_first_inputTxt" styleClass="textboxlarge" value="#{detailBeanName.pageDTO.regmaterialHeader}"/>
                <t:outputText value="*" styleClass="mandatoryAsterisk"/>
                <htm:br/> 
                <c2:requiredFieldValidator componentToValidate="edit_first_inputTxt" highlight="false" display="dynamic" errorMessage="#{globalResources.missingField}" active="#{regulationMaterialsMaintainBean.editDivShown}"/>
            </t:panelGroup>
            
            <t:outputText value="#{regResources.SubjectTextArea}" styleClass="lable01"/> 
            <t:panelGroup>
                <t:inputTextarea  value="#{detailBeanName.pageDTO.regmaterialText}"   forceId="true"  id="edit_inputTxtArea"  rows="4" cols="32"/>
                <t:outputText value="*" styleClass="mandatoryAsterisk"/>
                <htm:br/>
                <c2:requiredFieldValidator componentToValidate="edit_inputTxtArea" highlight="false" display="dynamic" errorMessage="#{globalResources.missingField}" active="#{regulationMaterialsMaintainBean.editDivShown}"/>
            </t:panelGroup>
